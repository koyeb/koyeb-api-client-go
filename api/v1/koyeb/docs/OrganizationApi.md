...
func main() {
    id := "id_example" // string | 
    body := *openapiclient.NewUpdateOrganizationNameRequest() // UpdateOrganizationNameRequest | 

    configuration := openapiclient.NewConfiguration()
    apiClient := openapiclient.NewAPIClient(configuration)
    resp, r, err := apiClient.OrganizationApi.UpdateOrganizationName(context.Background(), id).Body(body).Execute()
    if err != nil {
        fmt.Fprintf(os.Stderr, "Error when calling `OrganizationApi.UpdateOrganizationName``: %v\n", err)
        fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
    }
    // response from `UpdateOrganizationName`: UpdateOrganizationNameReply
    fmt.Fprintf(os.Stdout, "Response from `OrganizationApi.UpdateOrganizationName`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiUpdateOrganizationNameRequest struct via the builder pattern
