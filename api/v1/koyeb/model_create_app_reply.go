/*
Koyeb Rest API

The Koyeb API allows you to interact with the Koyeb platform in a simple, programmatic way using conventional HTTP requests. 

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package koyeb

import (
	"encoding/json"
)

// checks if the CreateAppReply type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CreateAppReply{}

// CreateAppReply struct for CreateAppReply
type CreateAppReply struct {
	App *App `json:"app,omitempty"`
}

// NewCreateAppReply instantiates a new CreateAppReply object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateAppReply() *CreateAppReply {
	this := CreateAppReply{}
	return &this
}

// NewCreateAppReplyWithDefaults instantiates a new CreateAppReply object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateAppReplyWithDefaults() *CreateAppReply {
	this := CreateAppReply{}
	return &this
}

// GetApp returns the App field value if set, zero value otherwise.
func (o *CreateAppReply) GetApp() App {
	if o == nil || IsNil(o.App) {
		var ret App
		return ret
	}
	return *o.App
}

// GetAppOk returns a tuple with the App field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateAppReply) GetAppOk() (*App, bool) {
	if o == nil || IsNil(o.App) {
		return nil, false
	}
	return o.App, true
}

// HasApp returns a boolean if a field has been set.
func (o *CreateAppReply) HasApp() bool {
	if o != nil && !IsNil(o.App) {
		return true
	}

	return false
}

// SetApp gets a reference to the given App and assigns it to the App field.
func (o *CreateAppReply) SetApp(v App) {
	o.App = &v
}

func (o CreateAppReply) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CreateAppReply) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.App) {
		toSerialize["app"] = o.App
	}
	return toSerialize, nil
}

type NullableCreateAppReply struct {
	value *CreateAppReply
	isSet bool
}

func (v NullableCreateAppReply) Get() *CreateAppReply {
	return v.value
}

func (v *NullableCreateAppReply) Set(val *CreateAppReply) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateAppReply) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateAppReply) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateAppReply(val *CreateAppReply) *NullableCreateAppReply {
	return &NullableCreateAppReply{value: val, isSet: true}
}

func (v NullableCreateAppReply) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateAppReply) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


